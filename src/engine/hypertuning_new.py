...
New-pipeline variant of ``hypertuning.py``. The public structure is kept identical
(``_suggest_params``, ``_objective``, ``refit``, ``optimize``); the only behavioural
change is that ``optimize`` builds the inner :class:`TimeSeriesCV` with the current
``(n_splits, train_size, test_size)`` API used by the new pipeline instead of the
old ``horizon`` argument.
"""
...
def optimize(study_name: str, X_train: pd.DataFrame, y_train: pd.Series, config: dict) -> None:
    ...
    Args:
        ...
        config (dict): Configuration dictionary for the study. Must provide the
            inner-CV keys ``n_inner_splits``, ``inner_train_size`` and
            ``inner_test_size`` used by the new :class:`TimeSeriesCV` API. May
            also provide ``gap`` (embargo in days, default 0) to prevent
            forward-looking target leakage; set it to the forecast horizon.
